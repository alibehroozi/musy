...
7. **Implement libs-first, one commit per layer.** Per AGENTS.md hard rule #10, each layer that's touched is its own commit. Empty layers are skipped.
   ...
   - **`feat(api): ...`** — `apps/api/src/modules/<name>/` with module + controller + service + repository + schema. Per `ARCHITECTURE.md` layering. **Tests in this layer call the real upstream HTTP client (per AGENTS.md hard rule #15).** Do not `jest.mock` SoundCloud / Deezer / Audius / Genius / etc. in any test — write the test as if integration. The only mocked clients are auth (Google OAuth and any later social-login). If the feature file's "Failure modes" section explicitly names a provider response that has to be forced, mock only that one client for that one test, and quote the authorizing line from the feature file in a comment so review can verify the override is real. If the provider needs a token / API key, surface it in the env cascade (hard rule #9) before the `feat(api):` commit lands — otherwise CI has no way to call the real upstream.
   ...
   - **`test(visual, web): user-flow spec for <feature>`** — a Playwright spec at `apps/web/tests/e2e/<feature-slug>.spec.ts` that replicates the user behavior described in the product-spec feature file. See step 7a below for the mapping. **Required whenever the feature touches `apps/web/` — skipped only for purely backend features. Authored per step 7a, never deferred.** If you reach step 8 without a `test(visual, web):` commit on a UI-touching feature, stop and back up — the layer is not optional, and a `feat(web):` commit without its paired `test(visual, web):` commit is an incomplete feature.
...
- **Reaching for `jest.mock` on a provider client.** AGENTS.md hard rule #15 forbids it outside auth + an explicit-spec-override. The smell is `jest.mock("./soundcloud.client")`, a hand-rolled `mockResolvedValue` for a music client, or a fixture file that pre-canned the upstream JSON. All of them ship a test that passes against a fiction. If the spec doesn't authorize the mock, the client is real — write the test against live HTTP and add the credential to the env cascade if needed.
