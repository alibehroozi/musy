...
| `/epic-plan`           | Plan a multi-feature epic: capture vision → explore → probe (design / behavior / value / tooling cost / privacy) → break down → draft self-contained feature specs under `.claude/pending-epics/<epic>/features/NN-<slug>.md`. Output is consumed by `/new-feature`.                            |
