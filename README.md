...
**With Claude Code:** run `/prepare-local` — it probes the prereqs, installs deps, copies missing `.env.local` files, brings up Docker, and tells you what's blank. Then `npm run dev`.

**Manually:**

```bash
npm install
cp apps/api/.env.example apps/api/.env.local   # backend secrets (Mongo, AI keys)
cp apps/web/.env.example apps/web/.env.local   # frontend build-time config
...
- **MongoDB** at `mongodb://localhost:27117` — connection string in `apps/api/.env.local` (`MONGO_URI`). The non-default port (`27117` instead of the standard `27017`) is intentional, so Docker doesn't collide with any other local mongo.
...
| --------------------- | ----------------------------- | ---------------------------------------------------------------------------------- |
| `apps/api/.env.local` | NestJS ConfigModule (runtime) | `MONGO_URI`, `ANTHROPIC_API_KEY`, OAuth secrets, session keys — server-only        |
| `apps/web/.env.local` | Vite (build time)             | `VITE_API_URL` and other `VITE_*` vars that get **inlined into the public bundle** |

`.env.local` is the local-dev convention (gitignored). NestJS reads `.env.local` first and falls back to `.env`; Vite loads `.env.local` automatically and lets it override `.env`. The `.env.example` files are the committed templates.

## Deployment

Each app deploys to a different surface:

- **Backend (`apps/api`)** → a runtime host (Render, Fly.io, Railway, AWS ECS, etc.). Set env values via the platform's secret manager. No `.env*` file in the build.
