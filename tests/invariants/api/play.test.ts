...
describe("API-12: /api/play/resolve self-heals stale-cache SoundCloud resolutions whose locator is no longer playable", () => {
  let h: PlayTestAppHandle | undefined;
  afterEach(async () => {
    if (h) await h.app.close();
    h = undefined;
  ...
  it("when a cached SC resolution can no longer produce a stream, re-runs lookup and returns the fresh playable URL", async () => {
    h = await buildPlayTestApp();
    const snapshot = makeSnapshot({ title: "Without Me", artist: "Eminem" });
    // Pre-seed the cache with a "stale" SC resolution that will no longer
    // resolve to a stream (simulating a track that became DRM-only after caching).
    const staleHash = await import("@moc/api-core").then((m) => m.computeSnapshotHash(snapshot));
    await h.repo.save(staleHash, snapshot, {
      source: "soundcloud",
      sourceTrackId: "stale-track-id",
      sourceLocator: "https://soundcloud.com/eminemofficial/without-me-album-version",
    });
    // Stale locator → produceStreamUrl returns null (the new selector refuses
    // DRM-only tracks). The fresh-lookup path returns a different locator
    // (a remix / user upload) whose produceStreamUrl yields a real mp3 URL.
    h.soundcloud.produceByLocator.set(
      "https://soundcloud.com/eminemofficial/without-me-album-version",
      null,
    );
    h.soundcloud.match = {
      sourceTrackId: "fresh-track-id",
      sourceLocator: "https://soundcloud.com/some-user/without-me-remix",
    };
    h.soundcloud.produceByLocator.set("https://soundcloud.com/some-user/without-me-remix", {
      streamUrl: "https://cf-media.sndcdn.com/fresh.128.mp3?Policy=...",
    ...
    expect(body.sourceTrackId).toBe("fresh-track-id");
    expect(body.streamUrl).toBe("https://cf-media.sndcdn.com/fresh.128.mp3?Policy=...");

    // The cache must have been rewritten with the fresh resolution so subsequent
    // requests hit the playable locator immediately.
    expect(h.repo.saved.at(-1)?.cached.sourceLocator).toBe(
      "https://soundcloud.com/some-user/without-me-remix",
    );
  });

  it("does not retry when a *fresh* lookup itself yields no playable candidate (avoids infinite loop)", async () => {
    h = await buildPlayTestApp();
    const snapshot = makeSnapshot({ title: "ObscurePhantomTrack", artist: "NoOne" });
    // No cache entry; lookup runs and finds nothing on either provider.
    h.audius.match = null;
    h.soundcloud.match = null;
    const res = await request(h.app.getHttpServer())
      .post("/api/play/resolve")
      .send({ snapshot })
      .set("Content-Type", "application/json");
    ...
    const body = ResolveResponse.parse(res.body);
    expect(body.streamUrl).toBeNull();
    // findMatch on each provider was called exactly once — no self-heal retry
    // when the response was generated by a fresh (non-cached) lookup.
    expect(h.audius.findCalls).toHaveLength(1);
    expect(h.soundcloud.findCalls).toHaveLength(1);
  });
});
