...
8. **Implement, libs-first, one commit per layer.** Per AGENTS.md hard rule #10 — same shape as `/new-feature`:
   ...
   - `change(api): ...` — controller / service / repository. **Tests in this layer hit the real upstream HTTP client (per AGENTS.md hard rule #15).** Do not introduce `jest.mock` on a music-provider client (SoundCloud, Deezer, Audius, Genius, …) as part of this change; if the existing tests already mock one without an authorizing line in the feature file, the right move is to **un-mock** them in this commit, not propagate the pattern. Auth clients remain the singular exception. A failure-mode test that genuinely needs a forced response shape may mock that one client iff the (possibly newly amended) feature file says so explicitly, and the test quotes the authorizing line in a comment.
...
- **Re-mocking provider clients during a change.** A behavior change can quietly stack a `jest.mock` on a SoundCloud / Deezer / Audius / Genius / etc. client to "stabilize" a flaky test. AGENTS.md hard rule #15 forbids it outside auth + an explicit-spec-override. If a real-upstream test goes flaky, fix flake at the source (retry, looser matcher, broader response shape) — never silence the upstream itself. If the change genuinely needs a forced response, amend the feature file's "Failure modes" first, then mock that one client in that one test with the authorizing line quoted in a comment.
